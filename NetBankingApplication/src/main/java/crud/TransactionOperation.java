...
public class TransactionOperation 
{
	...
	public List<Transaction> showTransactions(long accountNo, int limit)
	{
		...
		List<List<Object>> result= newOpt.selectMethod(objects, condition, order);
		
		List<Transaction> output= new ArrayList<Transaction>();
		if(result.size()!=0)
		{
			
			for(List<Object> rows: result)
			{
				output.add((Transaction)(rows.get(0)));
			}			
		}
