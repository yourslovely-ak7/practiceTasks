	...
	{		
		...
		try
		{
			...
			for (Map.Entry<T, List<String>> entry : objects.entrySet())
			{
				...
				{
					newObj= superClassFields(clazz,fields);
					requiredFieldObjects.add(newObj);
...
//			for(Map.Entry<Join,Join> entry : join.entrySet())
//			{
//				System.out.println(entry.getKey().getTableName()+" "+entry.getKey().getFieldName()+" : "+
//			entry.getValue().getTableName()+" "+ entry.getValue().getFieldName());
//			}
...
//			System.out.println(tableName+" "+fieldName+" "+currCond.getOperator()+" "+currCond.getValue());
	...
	{
		System.out.println("Join Method called");
		Map<Join,Join> join= new HashMap<>();
		String tableName= yaml.getTableName(classSimpleName);
...
//					tableNames.remove(referencedTable);
	...
	private Fields superClassFields(Class<?> childClazz, List<String> fields)
	{
		Fields newObj= new Fields();
			...
			for (Map.Entry<T, List<String>> entry : objects.entrySet())
			{
				...
				for(String currField: fields)
				{
					...
					if(row.keySet().contains(columnName))
					{
						...
						Method setterMethod= clazz.getDeclaredMethod(setterMethodName, parameterClass);
